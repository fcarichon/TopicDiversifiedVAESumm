...
class Vocab(object):
    ...
    def save(self, fpath):
        """Save vocabulary to the defined path"""
    ...
    def source2ids_ext(self, src_tokens, vocab_train_size):
    ...
    def outputids2words_bow(self, ids, BoW):
        ...
        for i in ids:
            try:
                if i== self.unk():
                    new_idx = torch.multinomial(BoW, 1)
                    w = self.id2word(new_idx)
                else:
    ...
    def outputids2words(self, ids, src_oovs):
        ...
        words = []
        extended_vocab = self.extend(src_oovs)
        #print('in_vocab : ', len(self._word_to_id))
